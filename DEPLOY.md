...
Сбор работает в Chromium, прокручивает внутреннее окно комментариев, сохраняет порции и раскрывает доступные ответы. Кнопка «Открыть Instagram» приостанавливает прокрутку и показывает браузер сервера в окне сайта: менеджер самостоятельно входит в Instagram, затем нажимает «Продолжить сбор». Сеанс Instagram сохраняется в томе приложения. Пароль передаётся по HTTPS в выбранное поле браузера и не записывается приложением в журнал.

Исходящее соединение с Instagram должно работать с сервера. Если необходим прокси, в Environment variables только Stack `marshrut-giveaway` добавьте `COLLECTOR_PROXY_SERVER` с протоколом и портом, а также `COLLECTOR_PROXY_USERNAME` / `COLLECTOR_PROXY_PASSWORD` при необходимости. HTTP/HTTPS поддерживает логин, SOCKS5 — без авторизации. Значения не добавляйте в Git. Nginx, общесерверный DNS и остальные сервисы для этого менять не нужно.

Если исходящее соединение пока не настроено, можно загрузить JSON из локального сборщика:
